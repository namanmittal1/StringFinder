...
{
    class SearchInXmlFile
    {
        int modeSelected = 0;
        StringComparison searchMode;

        public void ProcessXMLFile(string filename, String text)
        {
            try
            {
            ...
            try
            {
                ...
                foreach (string word in words)
                {
                    ...
                    if (matches.Count != 0)
                    {
                        ...
                        {
                            XMLFileData xfd = new XMLFileData();
                            xfd.Position_List = stringPostion1(line, text, incrementedval, searchMode);
                            incrementedval++;
                            xfd.Linenumber_List = linenumber;
                            xfd.FilenameList = filename;
                            xfd.Data_String = targetString;
                            System.Windows.Application.Current.Dispatcher.Invoke((Action)(() =>
                            {
                                XMLDataCollection_ViewModel.Instance.XMLData.Add(xfd);
                            }));
            ...
            try
            {
                ...
                foreach (string word in words)
                {
                    ...
                    if (matches.Count != 0)
                    {
                        ...
                        {
                            XMLFileData xfd = new XMLFileData();
                            xfd.Position_List = stringPostion2(line, text, incrementedval);
                            incrementedval++;
                            xfd.Linenumber_List = linenumber;
                            xfd.FilenameList = filename;
                            xfd.Data_String = targetString;
            ...
            try
            {
                ...
                foreach (string word in words)
                {
                    if (word.Equals(text, StringComparison.InvariantCultureIgnoreCase))
                    {
                        ...
                        XMLFileData xfd = new XMLFileData();
                        xfd.Position_List = stringPostion3(line, text, incrementedval, searchMode);
                        incrementedval++;
                        xfd.Linenumber_List = linenumber;
                        xfd.FilenameList = filename;
                        xfd.Data_String = targetString;
